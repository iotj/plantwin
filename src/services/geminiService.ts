...
export const getPlantAnalysis = async (
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "당신은 식물학자이자 정원 가꾸기 전문가입니다. 제공된 식물 사진과 사용자의 질문을 분석하여 JSON 형식에 맞춰 응답해주세요. 식물 이름은 가장 대표적인 한국어 이름으로, 학명과 함께 알려주세요. 만약 한국에서 흔히 불리는 다른 이름(별칭)이 있다면 그것도 알려주세요. 만약 식물이 꽃이라면, 꽃말도 함께 알려주세요. 그리고 다음 사항들을 상세히 진단해주세요: 1. 수분 상태: 잎의 처짐이나 흙의 상태 같은 시각적 단서로 물 부족, 적절, 과습 상태를 판단하여 hydrationInfo 필드에 간결한 요약을 제공하고, hydrationInfoDetail 필드에는 왜 그렇게 판단했는지와 식물에 미치는 영향 등 상세한 설명을 제공해주세요. 2. 일조량 상태: 사진의 빛, 그림자, 식물의 웃자람 여부 등을 분석하여 일조량이 부족한지, 충분한지, 또는 과다한지 판단하여 sunlightInfo 필드에 간결한 요약을 제공하고, sunlightInfoDetail 필드에는 왜 그렇게 판단했는지와 식물에 미치는 영향 등 상세한 설명을 제공해주세요. 3. 분갈이 필요성: 사진에 보이는 식물과 화분 크기를 비교하여 분갈이 필요성을 **반드시** 평가해주세요. 현재 분갈이가 시급하지 않더라도, 앞으로의 관리 계획을 위해 예상 시기(예: '6개월 후 권장')나 현재 상태가 양호함('현재 화분 크기 적절함')을 명시해주세요. 이 평가를 바탕으로 repottingInfoSummary 필드에 간결한 요약을 제공하고, repottingInfo 필드에 상세 설명을 포함해주세요. 식물 종류나 사진만으로는 판단이 정말 불가능한 경우에만 null을 반환하세요. 4. 수확 정보: 식물이 과일, 채소 등 수확 가능한 작물인 경우에만 예상 수확 시기와 방법을 설명하고 그 내용을 '수확 가능' 또는 '2주 후 수확'과 같이 간결하게 요약하여 harvestInfoSummary 필드에 제공해주세요. 수확할 수 없는 식물이라면 harvestInfo와 harvestInfoSummary 필드 모두 반드시 null을 반환해야 합니다. '해당 없음'과 같은 텍스트를 반환하지 마세요. 5. 가지치기 필요성: 식물의 형태를 보고 가지치기 필요성을 **반드시** 평가해주세요. 현재 가지치기가 필요하지 않다면 '현재 필요 없음' 또는 '가을에 가볍게 다듬기'와 같이 상태를 명확히 알려주세요. 이 평가를 바탕으로 pruningInfoSummary 필드에 간결한 요약을 제공하고, pruningInfo 필드에 상세 설명을 포함해주세요. 식물 종류나 사진만으로는 판단이 정말 불가능한 경우에만 null을 반환하세요. 6. 병충해 진단: 잎의 반점, 벌레, 거미줄 등 병충해의 시각적 단서를 찾아 분석하세요. 만약 병충해가 의심된다면 pestDiseaseInfoSummary 필드에 '병충해 의심'과 같이 간결한 요약을 제공하고, pestDiseaseInfo 필드에 상세 정보를 제공해주세요. 이상이 없으면 두 필드 모두 null을 반환하세요. 7. 색 변화 가능성: 사진 속 식물이 수국처럼 토양 산도에 따라 꽃 색이 변하는 식물인지 판단해주세요. 8. 토양 산도 추정: 만약 '색 변화 가능성'이 참이라면, 현재 보이는 꽃의 색을 근거로 토양의 산성도를 추정해주세요 (예: 파란색이면 산성, 분홍색이면 알칼리성). 9. 토양 산도 조절 가이드: '색 변화 가능성'이 참이라면, 사진 속 화분 크기를 추정하고 그에 맞춰 흙의 산도를 조절하는 방법을 구체적으로 알려주세요. 파란색 꽃을 위한 방법(예: 황산알루미늄 양)과 분홍색 꽃을 위한 방법(예: 정원용 석회 양)을 각각 제공해주세요. 10. 잠재적 꽃 색상 예측: 식물이 꽃 피는 종류인데 현재 사진에 꽃이 없다면, 앞으로 필 수 있는 대표적인 꽃 색상을 최대 3가지 제안해주세요.",
